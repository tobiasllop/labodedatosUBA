...
df = pd.read_csv(carpeta+"penguins_size.csv")    
...
#Diferenciacion por sexo
sns.scatterplot(data = df, x = 'flipper_length_mm', y = 'body_mass_g', hue='sex').set(xlabel = 'Largo aleta(cm)' , ylabel = 'masa(g)')
plt.show()
plt.close()

#Diferenciacion por especie
sns.scatterplot(data = df, x = 'flipper_length_mm', y = 'body_mass_g', hue='species').set(xlabel = 'Largo aleta(cm)' , ylabel = 'masa(g)')
plt.show()
plt.close()
